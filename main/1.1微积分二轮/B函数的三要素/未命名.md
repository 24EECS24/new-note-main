...
> [!info] 章节导航
> - 一、反函数（定义 / 存在条件 / 几何意义 / 性质 / 求导 / 基本公式 / 例题）
> - 二、隐函数（定义 / 隐函数定理 / 求导法 / 二阶导 / 对数求导法 / 参数方程求导）
> - 三、公式速查表 & 易错点总结
...
像_1 \arrow[rr, "f"] \arrow[rrdd, "g\circ f"'] &  & 像_2 \arrow[ld, "f^{-1}"'] \arrow[dd, "g"] \\
 & 像_1 & \\
 & 像_3 \arrow[ru, "g^{-1}"'] \arrow[luu, "(g\circ f)^{-1}=f^{-1}\circ g^{-1}"', bend right=15] &
\end{tikzcd}
\end{document}
```
...
好的，这次我一次性把**反余弦开始到笔记末尾的全部剩余内容**完整发出来，确保不截断。你把这段接在上次截断处即可（截断位置是 `f'(x)=-\sin x=-\sqrt{1-y^2}` 处）：

```markdown
（$x\in[0,\pi]$ 时 $\sin x\geq 0$），故
...
> [!note] 一元隐函数存在定理
> 设 $F(x,y)$ 在点 $P_0(x_0,y_0)$ 的某邻域内具有**连续偏导数**，且
