...
本页为空白页，仅存在少量印刷噪点、墨渍与无法辨识的透印痕迹，无有效可识别的高等数学笔记内容。
...
# 专升本数学严选800题
ZHUANSHENGBEN SHUXUE YANXUAN 800 T
## 第20题
【答案】$-\dfrac{3}{5}$
【解】
...
60. 【答案】 $\dfrac{\mathrm{e}^y}{1-x\mathrm{e}^y}$
...
61. 【答案】 $\dfrac{y-3x^2}{3y^2 - x}\mathrm{d}x$
...
专升本数学严选800题 ZHUANSHENGBENG SHUXUE YANXUAN 800 TI
...
## 基础部分答案解析
68. 【答案】$2x\int_{0}^{x} \cos t \mathrm{d}t + x^2 \cos x$
...
69. 【答案】$\mathrm{e}^{x^2 \ln x}\left(2x\ln x + x\right)\mathrm{d}x$
...
## 基础部分答案解析
本页为单栏排版，无分栏、无函数示意图，阅读顺序为从上至下，底部居中为页码。

81. 【答案】$-\frac{1}{3}\sin(1-3\mathrm{e}^x)+C$
【解】
$$
\begin{align*}
...
96. 【答案】$-\frac{2}{3}\mathrm{e}^{-3\sqrt{x}}+C$
【解】
$$
...
99. 【答案】$\frac{1}{2}x + \frac{1}{4}\sin 2x + C$
【解】
$$
\begin{aligned}
...
104. 【答案】$\dfrac{1}{2}\left[x^2 - 9\ln(9+x^2)\right] + C$
【解】
$$
\begin{aligned}
...
105. 【答案】$\dfrac{1}{3}\ln\left| \dfrac{x-2}{x+1} \right| + C$
【解】
$$
\begin{aligned}
...
106. 【答案】$\boldsymbol{\arccos \dfrac{1}{x} + C}$
...
107. 【答案】$\boldsymbol{\dfrac{1}{2}\arcsin \dfrac{2x}{3} + C}$
【解】
...
108. 【答案】$\boldsymbol{\sqrt{2x} - \ln(1+\sqrt{2x}) + C}$
...
109. 【答案】$\boldsymbol{\dfrac{1}{3}\sec^3 x - \sec x + C}$
【解】
$$
\begin{aligned}
...
\begin{tikzpicture}[x=1cm,y=1cm,scale=1.2]
    % 定义三个节点位置
    \node (lianxu) at (-1.5,0) {连续};
    \node (piandao) at (1.5,0) {可偏导};
    \node (kewei) at (0,-1.5) {可微};
    
    % 连续与可偏导之间：双向均不可推出，箭头中间标注叉号
    \draw[<->] (lianxu) -- (piandao) node[midway] {$\times$};
    
    % 可微可以推出连续，箭头从可微指向连续，叉号标注在箭头旁（三角形内侧），表示连续不可推出可微
    \draw[->] (kewei) -- (lianxu) node[midway, above right] {$\times$};
    
    % 可微可以推出可偏导，箭头从可微指向可偏导，叉号标注在箭头旁（三角形内侧），表示可偏导不可推出可微
    \draw[->] (kewei) -- (piandao) node[midway, above left] {$\times$};
\end{tikzpicture}
\end{document}
```
...
\begin{tikzpicture}[x=1cm,y=1cm]
    % 放置三个概念节点
    \node (lianxu) at (-2,0) {连续};
    \node (kepiandao) at (2,0) {可偏导};
    \node (kewei) at (0,-1.8) {可微};
    
    % 连续与可偏导：双向不可推导
    \draw[->] (lianxu) -- (kepiandao) node[midway,above] {$\times$};
    \draw[->] (kepiandao) -- (lianxu) node[midway,below] {$\times$};
    
    % 连续与可微：可微可推连续，连续不可推可微
    \draw[->] (kewei) -- (lianxu);
    \draw[->] (lianxu) -- (kewei) node[midway,left] {$\times$};
    
    % 可偏导与可微：可微可推可偏导，可偏导不可推可微
    \draw[->] (kewei) -- (kepiandao);
    \draw[->] (kepiandao) -- (kewei) node[midway,right] {$\times$};
\end{tikzpicture}
\end{document}
```
...
**专升本数学严选800题** ZHUANSHENGBEN SHUXUE YANXUAN 800 T
...
## 637. 【答案】$a=2$，$b=0$；$\displaystyle f'(x)=\begin{cases}
...
\begin{tikzpicture}[x=0.8cm,y=0.8cm,scale=1]
    ...
    % 抛物线 y²=2x，即x=y²/2
    \draw[line width=0.6pt, color=qqwuqq, smooth, samples=200, domain=-3:1]
        plot({(\y)^2/2}, \y) node[above right,font=\small] {$y^2=2x$};
    % 直线 y=-x+3/2，即x=3/2 - y
    \draw[line width=0.6pt, color=qqwuqq, dashed, samples=2, domain=-3:1]
        plot({3/2 - \y}, \y) node[below right,font=\small] {$y=-x+\frac{3}{2}$};
    % 填充阴影区域
    \fill[qqwuqq, opacity=0.3, smooth, samples=200]
        plot[domain=-3:1] ({(\y)^2/2}, \y) --
        plot[domain=1:-3] ({3/2 - \y}, \y) -- cycle;
\end{tikzpicture}
\end{document}
```
...
\begin{tikzpicture}[x=0.5cm,y=0.8cm,scale=1]
    ...
    % 标注点
    \node[below left,font=\small] at (0,0) {$O$};
    \node[below,font=\small] at (-exp(2),0) {$-\mathrm{e}^2$};
    \node[below,font=\small] at (1,0) {$1$};
    \node[below,font=\small] at (exp(2),0) {$\mathrm{e}^2$};
    % 曲线 y=lnx
    ...
    % 切线 y = x/e² +1
    \draw[line width=0.6pt, color=qqwuqq, smooth, samples=2, domain=-exp(2):exp(2)]
        plot(\x, {\x/exp(2) + 1}) node[above right,font=\small] {$y=\frac{1}{\mathrm{e}^2}x+1$};
    % 填充阴影区域
    \fill[qqwuqq, opacity=0.3]
        (-exp(2),0) --
        plot[domain=-exp(2):exp(2)] (\x, {\x/exp(2) +1}) --
        plot[domain=exp(2):1] (\x, {ln(\x)}) --
        cycle;
\end{tikzpicture}
\end{document}
```
...
765. 【证明】令$f(x)=x^3 + x^2 - x - 1$，在$[0,2]$上连续，
由于$f(0)=-1<0$，$f(2)=9>0$，
则由零点定理，知$\exists \xi \in (0,2)$，使得$f(\xi)=0$，$\xi$即为方程的根。

766. 【证明】令$f(x)=\ln(1+x)+x-1$，在$[0,1]$上连续，
...
797. 【证明】令$x=\frac{\pi}{2}-t$，$\mathrm{d}x=-\mathrm{d}t$，
...
798. 【证明】令$x=\pi-t$，$\mathrm{d}x=-\mathrm{d}t$，
则
$$
\begin{aligned}
...
## 强化部分答案解析
799. 【证明】令$t = \frac{1}{u}$，$\mathrm{d}t = -\frac{1}{u^2}\mathrm{d}u$，
则
...
800. 【证明】由拉格朗日定理，得$f(x) = f(x) - f(0) = f'(\xi) \cdot x$，$\xi \in (0,x)$，
从而
